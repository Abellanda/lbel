	...
	function site_pagination() {
        ...
        if ( $paginate_links ) {
            ...
            $preg_find = [
                ...
                "/<li class='active'><span aria-current='page'>(\d+)<\/span><\/li>/", // '(\d+)' Captures page number for backreference
            ...
            $preg_replace = [
                '',
                '',
                '<li class="pagination-previous"><a href="$1" aria-label="Previous page"><i class="material-icons">chevron_left</i>', // '$1' Outputs backreference href value
                '<li class="pagination-next"><a href="$1" aria-label="Next page"><i class="material-icons">chevron_right</i>', // '$1' Outputs backreference href value
                '<li class="active" aria-current="page"><span class="show-for-sr">You\'re on page </span>$1</li>', // '$1' Outputs backreference page number
            ...
            $paginate_links = '<div aria-label="Pagination">' . $paginate_links . '</div>';
...
if ( ! function_exists( 'site_get_the_comments_pagination' ) ) :
	function site_get_the_comments_pagination( $args = array() ) {
		$navigation = '';
		$args = wp_parse_args( $args, array(
			'prev_text'				=> __( '<i class="material-icons">chevron_left</i>', 'foundationpress' ),
			'next_text'				=> __( '<i class="material-icons">chevron_right</i>', 'foundationpress' ),
			'size'					=> 'default',
			'show_disabled'			=> true,
		) );
		$args['type'] = 'array';
...
// Custom Comments Pagination.
if ( ! function_exists( 'site_the_comments_pagination' ) ) :
	function site_the_comments_pagination( $args = array() ) {
		echo site_get_the_comments_pagination( $args );
	}
endif;
...
if ( ! function_exists( 'site_menu_fallback' ) ) :
	function site_menu_fallback() {
...
if ( ! function_exists( 'site_active_nav_class' ) ) :
	function site_active_nav_class( $classes, $item ) {
		if ( $item->current == 1 || $item->current_item_ancestor == true ) {
			$classes[] = 'is-active';
		}
		return $classes;
	}
	add_filter( 'nav_menu_css_class', 'site_active_nav_class', 10, 2 );
endif;
 ...
 */
if ( ! function_exists( 'site_active_list_pages_class' ) ) :
	function site_active_list_pages_class( $input ) {
	...
	add_filter( 'wp_list_pages', 'site_active_list_pages_class', 10, 2 );
endif;
...
if ( ! function_exists( 'site_mobile_menu_id' ) ) :
	function site_mobile_menu_id() {
...
if ( ! function_exists( 'site_title_bar_responsive_toggle' ) ) :
	function site_title_bar_responsive_toggle() {
 ...
 */
if ( ! function_exists( 'site_gallery' ) ) :
	function site_gallery($attr) {
	...
	add_shortcode('gallery', 'site_gallery');
endif;
